...
public class Comments extends ListActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        String mname = in1.getStringExtra("mname");
        

		try
		{
		httpclient = new DefaultHttpClient();
		httppost = new HttpPost("http://10.0.2.2/vcmt.php");
        // Add your data
				nameValuePairs = new ArrayList<NameValuePair>(1);
				nameValuePairs.add(new BasicNameValuePair("Moviename", mname));
...
//				Toast.makeText(getApplicationContext(), temp[12]+""+temp[13]+""+temp[14]+""+temp[15]+"", Toast.LENGTH_LONG).show();
        ...
        for (int i = 1; i <=lnum;i++) {
			// creating new HashMap
				...
				map.put(KEY_NAME, temp[cn]);
				map.put(KEY_COMM,KEY_COMM+temp[cc]);
				map.put(KEY_RATE,KEY_RATE+temp[cr]);
